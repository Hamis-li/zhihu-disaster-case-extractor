...
| 🏆 赛事项目页 | https://www.zhihu.com/hackathon/project/90033 |
| 🎬 演示视频 | https://www.bilibili.com/video/BV1KAe56TEt2/ |
| 📝 知乎投稿 | https://zhuanlan.zhihu.com/p/2082410743996195009 |
| 🐙 GitHub | https://github.com/Hamis-li/zhihu-disaster-case-extractor |
| 📊 最终热度 | 200 |
| ⭐ 评审得分 | 5.53 |
...
- **应用框架**：Streamlit（单文件应用 `app.py`，约 1400 行）
...
在 `.streamlit/secrets.toml`（本地，已被 .gitignore 排除）或 Streamlit Cloud 的 **Settings → Secrets** 中配置。

> **TOML 格式要求**：每行一个 `键 = "值"`，等号后不能换行。

| 变量 | 必填 | 说明 |
|---|---|---|
| `ZHIHU_ACCESS_SECRET` | 是 | 知乎数据开放平台个人中心申请的 Access Secret，用于搜索 API 与直答 Agent API |
| `ZHIHU_OAUTH_APP_ID` | OAuth 登录需要 | 知乎 OAuth 应用 App ID |
| `ZHIHU_OAUTH_APP_KEY` | OAuth 登录需要 | 知乎 OAuth 应用 App Key |
| `ZHIHU_OAUTH_REDIRECT_URI` | OAuth 登录需要 | 授权回调地址，必须与开放平台登记值逐字符一致 |

可选覆盖项（一般使用默认值即可）：`ZHIHU_API_BASE`、`ZHIHU_SEARCH_PATH`、`ZHIHU_AGENT_PATH`、`ZHIHU_QUOTA_PATH`、`ZHIHU_OAUTH_AUTHORIZE`、`ZHIHU_OAUTH_TOKEN`、`ZHIHU_OAUTH_USER`。

> **OAuth 申请通道**：发邮件至 openplatform@zhihu.com，申请材料需含应用名称、简介、≥256×256 图标、授权回调地址、申请人信息；`redirect_uri` 必须与申请 / 赛事登记值**逐字符一致**（协议、域名、路径、末尾斜杠都要相同）。
...
3. 在应用 **Settings → Secrets** 中添加 `ZHIHU_ACCESS_SECRET`（搜索 / 直答）以及 `ZHIHU_OAUTH_APP_ID`、`ZHIHU_OAUTH_APP_KEY`、`ZHIHU_OAUTH_REDIRECT_URI`（登录）
...
│
├── .streamlit/
│   ├── config.toml                     # Streamlit 主题配置（知乎蓝）
...
│   └── brand/
│       └── project_icon.png            # 项目封面图（OAuth 申请 & 赛事用）
│
├── brand/
│   ├── content.md                      # 品牌策略与文案规范
│   └── brand_identity/
│       └── brand_identity.html         # 品牌识别手册（可视化）
│
├── zhihu_blue_theme/
│   ├── palette.html                    # 配色方案可视化色板
│   ├── .streamlit/config.toml          # 主题配置参考
│   └── assets/custom.css               # 自定义样式参考
│
├── docs/
│   ├── 知危鉴_项目计划书.pdf            # 项目计划书（最终版）
│   ├── 演示视频脚本.md                  # 产品演示视频分镜脚本
│   ├── OAuth申请邮件_草稿.txt           # OAuth 应用申请邮件模板
│   ├── 赛事竞品分析报告.html            # 150+ 参赛项目竞品分析
│   └── 公众号推文.html                  # 微信公众号推文（可直接粘贴发布）
```
